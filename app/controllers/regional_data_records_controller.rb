class RegionalDataRecordsController < SuperuserController
  ...
  def new
    @regional_data_record = @region.regional_data_records.new
  end
  ...
  def update
    respond_to do |format|
      if @regional_data_record.update(regional_data_record_params)
        format.html { redirect_to [@region, @regional_data_record], notice: 'Regional data record was successfully updated.' }
  ...
  private
    ...
    def regional_data_record_params
      params.require(:regional_data_record).permit(:region_id, :release_date, :acadyr, :cost_per_stud, :private_nonsec, :private_sec, :special_HEI, :other_govt_schools, :ched_supervised_institution, :local_colleges_and_uni, :SUCs_main, :SUCs_sat)
    end
end
